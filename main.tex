...
\begin{frame}[fragile]
	...
	\tikz[overlay,remember picture]\node[anchor=north east] at (current page.north east) {\scalebox{.7}{\usebox\issue}};
	\begin{minipage}{.6\textwidth}
		\begin{tikzpicture}
			\node[outer sep=0pt](F){\begin{dottedborder}
				\begin{minted}{java}
					if(arr[i] > arr[j])
						// ...
				\end{minted}
			\end{dottedborder}};
			\node[outer sep=0pt,below=of F](F2){\begin{dottedborder}
				\begin{minted}{java}
				...
				\end{minted}
			\end{dottedborder}};
			\node[anchor=center,yshift=1.4mm] at (F.south) {\uselabel{want}};
			\node[anchor=center,yshift=1.4mm] at (F2.south) {\uselabel{got}};
		\end{tikzpicture}
	\end{minipage}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.45\textwidth}
			\begin{tikzpicture}
				...
				\coordinate (C) at ($(narg)!0.5!(n)$);
				\coordinate[above=of n] (T);
				\node[um=<{1,3-9}>,gn,anchor=south] (intn) at (C |- T) {int n};
