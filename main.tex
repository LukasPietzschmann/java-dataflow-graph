...
\begin{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.35\textwidth}
			\begin{description}
				\item[\javainline{flowsTo(b)}]\hfill\\ Traverse all paths that lead to b.
				\item[\javainline{flowsFrom(a)}]\hfill\\ Traverse all paths start start at a.
				\item[\javainline{dependsOn(a, b)}]\hfill\\ Is there a path between a and b?
			\end{description}
		\end{column}
		\begin{column}{0.1\textwidth}
			\begin{tikzpicture}
				...
				\draw[read] (m) to (intm);
				\coordinate[below=of e,yshift=4mm] (o);
				\node[doutline,fit=(intm)(m)(e)(o),outer sep=0pt,fill=green!50!gray!5,draw=green!50!gray] (F) {};
				\node[vf=15] at (F.south) {\uselabel{addFive}};
			\end{tikzpicture}
		\end{column}
		\begin{column}{0.45\textwidth}
			\begin{tikzpicture}
				\node[gn] (narg)                          {n};
				\node[gn,xshift=3cm] (n) [right=of narg]  {n};
				\coordinate (C) at ($(narg)!0.5!(n)$);
				\coordinate[above=of n] (T);
				\node[gn,anchor=south] (intn) at (C |- T) {int n};
				\node[gn] (addFive) [below=of narg]       {addFive(n)};
				\node[gn] (xthen) [below=of addFive]      {x = addFive(n)};
				\node[gn] (xelse) at (n |- xthen)         {x = n};
				\coordinate[below=of xelse] (B);
				\node[gn, anchor=north](returnx)at(C |- B){return x};
				\node[gn] (intx) at (C |- xelse)          {int x = 42};
				\node[gn] (b) at (C |- addFive)           {b};
				\node[gn] (boolb) at (C |- n)             {boolean b};

				\draw[read] (returnx) to (xelse);
				\draw[occ] (xelse) to (intx);
				\draw[con] (xelse) to (b);
				\draw[read] (returnx) to (xthen);
				\draw[occ] (xthen) to (intx);
				\draw[con] (xthen) to (b);
				\draw[read] (n) to (intn);
				\draw[read] (narg) to (intn);
				...
				\coordinate[below=of returnx,yshift=4mm] (b);
				\node[doutline,fit=(intn)(returnx)(xthen)(xelse)(b),outer sep=0pt,fill=blue!50!gray!5,draw=blue!50!gray] (q) {};
