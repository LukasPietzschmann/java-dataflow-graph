...
\begin{frame}
	...
	\savebox\stepi{\begin{tikzpicture}[node distance=1ex]
		\node[inline] (On)   {\javainline{flowsTo(...)}};
		\node[inline] (From) [below=of On.south east,anchor=north east] {\javainline{flowsFrom(...)}};
		\node[inline] (To)   [below=of From.south east,anchor=north east] {\javainline{dependsOn(...)}};
	\end{tikzpicture}}
...
\begin{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.35\textwidth}
			...
			\begin{description}
				\item<1>[\javainline{flowsTo(a)}]\hfill\\ Traverse all paths that lead to \cgn{a}.
				\item<1->[\javainline{flowsFrom(}\alt<1>{\javainline{a)}}{\javainline{param_n)}}]\hfill\\ Traverse all paths start start at \cgn{a}.
				\item<1>[\javainline{dependsOn(a, b)}]\hfill\\ Is there a path between \cgn{a} and \cgn{b}?
			\end{description}
		\end{column}
		\begin{column}{0.1\textwidth}
			\begin{tikzpicture}
				\node[um=1,gn] (intm) {int m};
				\node[um=1,gn] (m) [below=of intm] {m};
				\node[um=1,gn] (e) [below=of m] {m + 5};
				\draw[um=1,def] (e) to (m);
				\draw[um=1,read] (m) to (intm);
				\coordinate[below=of e,yshift=4mm] (o);
				\node[um=1,doutline,fit=(intm)(m)(e)(o),outer sep=0pt,fill=green!50!gray!5,draw=green!50!gray] (F) {};
				\node[um=1] at (F.south) {\uselabel{addFive}};
			\end{tikzpicture}
		\end{column}
		\begin{column}{0.45\textwidth}
			\begin{tikzpicture}
				\node[um=1,gn] (narg)                          {n};
				\node[um=1,gn,xshift=3cm] (n) [right=of narg]  {n};
				\coordinate (C) at ($(narg)!0.5!(n)$);
				\coordinate[above=of n] (T);
				\node[um=1,gn,anchor=south] (intn) at (C |- T) {int n};
				\node[um=1,gn] (addFive) [below=of narg]       {addFive(n)};
				\node[um=1,gn] (xthen) [below=of addFive]      {x = addFive(n)};
				\node[um=1,gn] (xelse) at (n |- xthen)         {x = n};
				\coordinate[below=of xelse] (B);
				\node[um=1,gn, anchor=north](returnx)at(C |- B){return x};
				\node[um=1,gn] (intx) at (C |- xelse)          {int x = 42};
				\node[um=1,gn] (b) at (C |- addFive)           {b};
				\node[um=1,gn] (boolb) at (C |- n)             {boolean b};

				\draw[um=1,read] (returnx) to (xelse);
				\draw[um=1,occ] (xelse) to (intx);
				\draw[um=1,con] (xelse) to (b);
				\draw[um=1,read] (returnx) to (xthen);
				\draw[um=1,occ] (xthen) to (intx);
				\draw[um=1,con] (xthen) to (b);
				\draw[um=1,read] (n) to (intn);
				\draw[um=1,read] (narg) to (intn);
				\draw[um=1,read] (addFive) to (narg);
				\draw[um=1,def] (xthen) to (addFive);
				\draw[um=1,def] (xelse) to (n);
				\draw[um=1,read] (b) to (boolb);

				\coordinate[below=of returnx,yshift=4mm] (b);
				\node[um=1,doutline,fit=(intn)(returnx)(xthen)(xelse)(b),outer sep=0pt,fill=blue!50!gray!5,draw=blue!50!gray] (q) {};

				\node[um=1] at (q.south) {\uselabel{magic}};
			\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
