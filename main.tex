...
\begin{frame}
	...
	\savebox\stepi{\begin{tikzpicture}[node distance=1ex]
		...
		\node[inline] (To)   [below=of From.south east,anchor=north east,opacity=0.3] {\mintinline{java}{flowsTo(...)}};
	\end{tikzpicture}}
	\tikz[overlay,remember picture]\node[roundednode, draw=gray, anchor=north east, outer sep=1ex, inner sep=1ex] at (current page.north east) {\usebox\stepi};
\end{frame}
...
\begin{frame}
	...
	\savebox\stepii{\begin{tikzpicture}[node distance=1ex]
		\node[coolgraphnode,xshift=2mm] (L1) {};
		\node[coolgraphnode] (L2) [right=of L1, xshift=1ex] {};
		\node[coolgraphnode,yshift=1cm] (R) at ($(L1)!0.5!(L2)$) {};
		\foreach \i in {L1,L2} \draw[arrow, line cap=round, shorten >=1mm,shorten <= 1mm] (R) to (\i);
	\end{tikzpicture}}
	\tikz[overlay,remember picture]\node[roundednode, draw=gray, anchor=north east, outer sep=1ex, inner sep=1ex] at (current page.north east) {\usebox\stepii};
\end{frame}
...
\begin{frame}
	...
	\savebox\stepiii{\begin{tikzpicture}[node distance=1ex]
		\node[coolgraphnode,xshift=2mm] (L21) {};
		\node[coolgraphnode] (L22) [right=of L21, xshift=1ex] {};
		\node[coolgraphnode,yshift=1cm] (R2) at ($(L21)!0.5!(L22)$) {};
		...
		\draw[arrow,dashed] (L2E) to[out=90,in=0] node[roundnode,inner sep=0.3pt,solid]{\tiny4} (RE);
	\end{tikzpicture}}
	\tikz[overlay,remember picture]\node[roundednode, draw=gray, anchor=north east, outer sep=1ex, inner sep=1ex] at (current page.north east) {\usebox\stepiii};
\end{frame}
